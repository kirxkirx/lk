...
void normalize_spectral_window_file(unsigned long int N_freq){
 ...
 max_W=window[0];
 for(i=0;i<N_freq;i++){
  if( max_F<theta[i] )max_F=theta[i];  
  if( max_W<window[i] )max_W=window[i];
 }
...
int main(int argc, char **argv){
 ...
 FILE *lcfile;
 FILE *LK_periodogramfile;
 FILE *DFT_periodogramfile;
 ...
 // TBA: range check!
 if( pmax<0.0 ){
  fprintf(stderr, "ERROR: pmax<0.0 \n");
  return 1;
 }
 if( pmin<0.0 ){
  fprintf(stderr, "ERROR: pmin<0.0 \n");
  return 1;
 }
 if( step<0.0 ){
  fprintf(stderr, "ERROR: step<0.0 \n");
  return 1;
 }
 if( step>0.5 ){
  fprintf(stderr, "ERROR: step>0.5 \n");
  return 1;
 }
 double ptemp;
 if( pmin>pmax ){
 ...
 if( pmin==pmax ){
  fprintf(stderr, "ERROR: pmin==pmax \n");
  return 1;
 }
 ...
 if( argc==6 ){
  ...
  // Check range!
  if( shuffle_iterations<0 ){fprintf(stderr,"ERROR: the number of shuffle iteration cannot be <0\n");return 1;}
  if( shuffle_iterations>1000000 ){fprintf(stderr,"ERROR: the number of shuffle iteration cannot be >1000000\n");return 1;}
 }
 if( shuffle_iterations>0 )fprintf(stderr,"Lightcurve shuffle iterations: %d\n",shuffle_iterations);
 ...
 if( lcfile==NULL ){fprintf(stderr,"ERROR opening the lightcurve file %s\n",argv[1]);return 1;}
 ...
 for(N_obs=1;;N_obs++){
  ...
  if( N_obs==MAX_NUMBER_OF_OBSERVATIONS ){fprintf(stderr,"ERROR: too many lines in the input file >%d\n",MAX_NUMBER_OF_OBSERVATIONS);return 1;} // just in case it is infinite length
 }
 fseek(lcfile,0,SEEK_SET); // go back to the beginning of the lightcurve file

 jd=malloc(N_obs*sizeof(double));
 if( jd==NULL ){fprintf(stderr,"ERROR: allocating memory for the jd array!\n");fclose(lcfile);return 1;}
 m=malloc(N_obs*sizeof(double));
 if( m==NULL ){fprintf(stderr,"ERROR: allocating memory for the m array!\n");fclose(lcfile);return 1;}
 if( compute_Deeming==1 ){
  ...
  if( compute_Deeming==1 && shuffle_iterations==0 ){
   ...
   if( m_fake==NULL ){fprintf(stderr,"ERROR: allocating memory for the m_fake array!\n");fclose(lcfile);return 1;}
 ...
 */
 ...
 // +1 as we always want at leat one iteration - the run at the original non-shuffled lightcurve
 for(shuffle_iteration=0;shuffle_iteration<shuffle_iterations+1;shuffle_iteration++){
  ...
  for(i=0;i<N_freq;i++){
   freq[i]=fmin+i*df;
   if( compute_Deeming==1 )compute_DFT( jd, m, N_obs, freq[i], &power[i], T_DFT);
   if( compute_Deeming==1 && shuffle_iterations==0 )compute_DFT( jd, m_fake, N_obs, freq[i], &spectral_window[i], T_DFT);
   if( compute_LK==1 )theta[i]=compute_LK_reciprocal_theta( jd, m, N_obs, freq[i], 0.0); // mean mag is always 0.0 as we subtracted it from the LC
 ...
 if( compute_Deeming==1 ){
  fprintf(stdout,"%lf %lf",noshuffle_DFT_periodogram_max_freq,noshuffle_DFT_periodogram_max );
  if( shuffle_iterations>0 )fprintf(stdout,"  %lf +/- %lf",DFT_p, sqrt((double)DFT_shuffled_peak_counter)/(double)shuffle_iterations );
  fprintf(stdout," DFT\n" );
 }
 if( compute_LK==1 ){
  fprintf(stdout,"%lf %lf",noshuffle_LK_periodogram_max_freq,noshuffle_LK_periodogram_max );
  if( shuffle_iterations>0 )fprintf(stdout,"  %lf +/- %lf",LK_p, sqrt((double)LK_shuffled_peak_counter)/(double)shuffle_iterations );
  fprintf(stdout," LK\n" );
 }
 ...
 if( shuffle_iterations==0 ){
  ...
  if( compute_LK==1 ){
   ...
   if( NULL==LK_periodogramfile ){fprintf(stderr,"ERROR writing lk.periodogram\n");return 1;}
  }
  if( compute_Deeming==1 ){
   DFT_periodogramfile=fopen("deeming.periodogram","w");
   if( NULL==DFT_periodogramfile ){fprintf(stderr,"ERROR  writing deeming.periodogram\n");return 1;}
  }
  for(i=0;i<N_freq;i++){
   if( compute_LK==1 )fprintf(LK_periodogramfile,"%5.10lf %5.10lf\n",freq[i],theta[i]);
   if( compute_Deeming==1 )fprintf(DFT_periodogramfile,"%5.10lf %5.10lf %5.10lf\n",freq[i],power[i],spectral_window[i]);
  }
  if( compute_LK==1 )fclose(LK_periodogramfile);
  if( compute_Deeming==1 ){
 ...
 free(freq);
 if( compute_LK==1 )free(theta);
 if( compute_Deeming==1 )free(power);
 if( compute_Deeming==1 && shuffle_iterations==0 ){
  free(m_fake);
